...
| `deno task codex:template [-- --apply]`               | Dry-run by default. Applies `.config/codex-template/{config.toml,AGENTS.md}` to `~/.codex` only with `--apply` (`template` has no markers; `~/.codex/config.toml` must contain marker block). |

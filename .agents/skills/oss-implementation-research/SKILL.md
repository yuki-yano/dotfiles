...
description: GitHub リポジトリや既存 OSS の実装を調査するときに使う。
---
...
- `--cwd` の意味と cache の扱いは `references/storage-policy.md` を参照。
...
- `sources.json` の `path` の扱いは `references/storage-policy.md` を参照。

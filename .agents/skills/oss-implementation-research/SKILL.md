...
cd "$repo_root"
npx -y opensrc@latest honojs/hono --cwd tmp/ai/references --modify=false
npx -y opensrc@latest vercel/ai#main --cwd tmp/ai/references --modify=false
npx -y opensrc@latest pypi:requests crates:serde --cwd docs/references --modify=false
```
...
```bash
bash .agents/skills/oss-implementation-research/scripts/prepare_oss_reference.sh honojs/hono --target-dir tmp/ai/references/git-clones
bash .agents/skills/oss-implementation-research/scripts/prepare_oss_reference.sh honojs/hono --target-dir docs/references/git-clones --ref v4.9.7
```

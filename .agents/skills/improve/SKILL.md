...
| モード | 範囲 | subagent |
|---|---|---|
| `quick` | 高変更頻度または高リスク箇所のcorrectness、security、tests | 原則なし。大規模な場合のみ1件 |
| 標準 | 主要packageを中心に全カテゴリ | 独立調査が有効な場合に最大3件 |
| `deep` | 全packageをカテゴリごとに段階調査 | 1wave最大3件。利用可能枠を超えない |
| `branch` | default branchとのmerge-base以降の変更と直接の呼び出し元 | 原則なし |
| 分野指定 | `security`、`perf`、`tests`など指定カテゴリのみ | 必要な場合だけ |
| `next` | 今後の方向性だけを調査 | 必要な場合だけ |
| `reconcile` | 既存の改善バックログを再検証 | 原則なし |
...
subagentを使う場合は、相互に独立したカテゴリまたはpackageへ分け、read-onlyの調査だけを任せる。
各subagentへ次を渡す。

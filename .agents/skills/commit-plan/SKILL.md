---
name: commit-plan
description: Git差分を意図別に整理し、コミット計画を作成して安全に実行する。ユーザーが「コミットを分けたい」「変更を論理的に整理したい」「段階的にコミットしたい」「実行前に計画を確認したい」「commitして」と依頼するときに使う。
---
...
計画フェーズと実行フェーズを分離し、原則として 1 文字入力で承認を取る。
ユーザーが「計画だけ」「実行しない」と明示した場合は、計画を提示して終了し、実行確認は出さない。
ユーザーが「commitして」「コミットして」「commit までやって」のように明示的にコミット実行を依頼した場合は、計画提示後の 1 文字確認を省略して Phase 2 を開始する。
`y` 承認後、または明示的なコミット実行依頼がある場合は、計画した全コミットを順次実行する。各コミット前の追加確認は行わない。
...
- 対象タイプ（`--type <type>`）
- グルーピング方針（まとめる / 分ける）
- 実行意図（計画のみ / 承認後に実行 / 明示指示により確認なしで実行）
...
### Step 3: 実行確認または自動実行の分岐を行う

計画のみの依頼では、確認を出さずに終了する。

明示的なコミット実行依頼がある場合は、計画提示後に次を伝えて Phase 2 を開始する。

```text
明示的な commit 指示があるため、確認を省略して Phase 2 を開始します。
```

ただし、停止条件に該当する差分や確認が必要な差分がある場合は、明示的なコミット実行依頼があっても Phase 2 に進めず、停止してユーザー確認する。

それ以外で実行意図がある場合、計画提示後は以下で確認する。
...
`y` が来た場合、または明示的なコミット実行依頼がある場合のみ、`references/execution-and-patch.md` の手順で実行する。

- バックアップブランチ作成
- コミットを順次実行
- 各コミット後の検証
- 計画との差分不一致、patch 適用失敗、コミット失敗、検証失敗があれば停止して報告

...
description: スマホに ntfy でプッシュ通知を送信する。ユーザーが「スマホ通知も」「スマホにも通知」など明示的に要求したときに使う。
---
...
タイトルは必ず `[$REPO_NAME] {Codex|Claude Code} ${実行内容の要約}` 形式にする。`--title` で完全なタイトルを指定する場合も、この `[$REPO_NAME]` プレフィックスを必須とし、一致しない場合はエラー終了する。
...
- `--agent` や `NTFY_NOTIFY_AGENT` に `codex|claude` 以外を指定するとエラー終了する。

## 実行手順

1. `NTFY_TOPIC` を設定する。
2. `--summary` に 100-200 文字の要約を指定する。
3. 必要なら `--agent` で通知主体を指定する（詳細は「実行主体の切替」を参照）。
...
- `NTFY_NOTIFY_AGENT`: 任意。通知主体を指定する（詳細は「実行主体の切替」を参照）。
...
- `--title`: 任意。完全なタイトルを指定する（形式は「タイトル規約」を参照）。

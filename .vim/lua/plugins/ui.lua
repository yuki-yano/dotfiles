...
return {
  ...
  },
  {
    ...
    config = function()
      require('scope').setup()
      local bufferline_config = require('plugins.config.bufferline')
      bufferline_config.setup()

      local group = vim.api.nvim_create_augroup('BufferlineTransparency', { clear = true })
      vim.api.nvim_create_autocmd({ 'ColorScheme' }, {
        group = group,
        callback = function()
          bufferline_config.refresh()
        end,
      })
      vim.api.nvim_create_autocmd({ 'User' }, {
        group = group,
        pattern = 'TransparencyApplied',
        callback = function()
          bufferline_config.refresh()
        end,
      })
